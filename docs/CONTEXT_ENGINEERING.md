...
- [Agents](../rivalens/agents): 规划、采集、结构化、分析、撰写、发布 Agent。
- [Evidence Collector](../rivalens/research/evidence_collector.py): CollectionAgent 使用的 ResearchEngine 证据采集适配器。
- [Branch Review](../rivalens/agents/branch_review.py): CollectionAgent 的分支展开审查器，控制 research branch 是否继续展开。
...
- 不要恢复末端“删 claim 即通过”的伪质检闭环。新的 review 逻辑应优先在 standard search 后作为证据质量门设计。
