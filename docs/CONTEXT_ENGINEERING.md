...
- [Coverage Review](../rivalens/agents/coverage_review.py): CollectionAgent 的覆盖审查器，输出 source type / guiding question 缺口和 gap-driven follow-up tasks。
- [Claim Support Review](../rivalens/agents/claim_support.py): analysis 后的 claim-level citation support gate，可生成一次 claim-driven verification queue。
