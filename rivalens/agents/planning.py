...
def _planning_trace_outputs(output: Any) -> dict[str, Any]:
    ...
    selected_industry = industry_direction_plan.get("industry") or {}
    ...
    return {
        ...
        "candidate_industry_count": len(
            industry_direction_plan.get("candidate_industries", []),
        ),
...
class PlanningAgent:
    ...
    async def _run_scope_planner(
        ...
        selected_industry = industry_direction_plan.get("industry") or {}
        candidate_industries = industry_direction_plan.get("candidate_industries", [])

        research_artifacts = state.get("research_artifacts", []) + [
            {
                "id": "artifact_planning_research_plan_1",
                "agent": "planner",
                "mode": "research_plan",
                "query": query,
                "report": (
                    "Selected industry "
                    f"{selected_industry.get('name', 'unknown industry')} and "
                    f"{len(analysis_dimensions)} analysis dimensions."
                ),
                "context": {
        ...
        return {
            ...
            + [
                {
                    "agent": "planner",
                    "action": "normalize_scope_and_plan_research",
    ...
    def _planning_query(self, query: str, file_context: dict[str, Any]) -> str:
        ...
        return "\n".join(
            [
                query,
                "",
                "User-provided local file context for industry and analysis planning:",
                summary,
                "Search and analysis hints:",
