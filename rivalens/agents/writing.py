...
"""
    ...
    def _dynamic_analysis_sections(
        ...
        for evidence in evidence_items:
            dimension_id = str(evidence.get("dimension_id", "") or "")
            if (
                dimension_id
        ...
        for index, dimension_id in enumerate(
            ...
            section_evidence = [
                evidence
                for evidence in evidence_items
                if evidence.get("dimension_id") == dimension_id
            ]
            sections.append(
                {
                    "number": f"3.{index}",
    ...
    def _claim_dimension_id(
        ...
        dimension_id = str(claim.get("dimension", "") or "")
        if dimension_id and dimension_id != "competitor_profile":
            return dimension_id
        for evidence_id in claim.get("evidence_ids", []):
            evidence = evidence_by_id.get(evidence_id, {})
            evidence_dimension = str(evidence.get("dimension_id", "") or "")
    ...
    def _matches_dynamic_section(
        ...
        item_dimension_ids: list[str] = []
        for key in ("dimension", "dimension_id"):
            dimension_id = str(item.get(key, "") or "")
            if dimension_id:
                item_dimension_ids.append(dimension_id)
            if dimension_id and dimension_id in source_dimension_ids:
                return True
        if not item_dimension_ids and "evidence_supported_findings" in source_dimension_ids:
            return True
        return False
