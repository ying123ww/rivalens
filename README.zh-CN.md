...
- **多检索器搜索** — 每个收集任务可配置检索器链（Tavily、UniFuncs DeepSearch、Serper、Exa、MCP、本地等）
...
LangGraph 入口为 `rivalens/workflows/agent.py`，当前多智能体 DAG：
...
| **数据迁移** | Alembic |
| **包管理** | Poetry |
| **LLM 提供商** | OpenAI, Anthropic, Ollama, Groq, Google GenAI, XAI, Mistral, Cohere, Together, Fireworks, DashScope, GigaChat (通过 LangChain 适配器) |
| **搜索检索器** | Tavily, UniFuncs DeepSearch, Serper, Exa, DuckDuckGo, Arxiv, MCP |
| **页面爬取** | BeautifulSoup4, lxml, Playwright, Selenium, Scrapy, PyMuPDF |
...
```bash
python cli.py "你的研究问题" --report_type detailed_report --tone objective
```
...
| PUT | `/api/sessions/{session_id}/memory` | 更新会话记忆（消息历史） |
...
| 方法 | 路径 | 说明 |
|--------|------|-------------|
| GET | `/api/trace/runs/{run_id}` | 获取可视化工作流图及完整溯源数据包 |
...
**source_collection**（CollectionAgent）将确认的分析维度展开为"竞品 × 维度"收集分支，通过 `ResearchEngineEvidenceCollector` 并发执行。为每个选定竞品创建 `competitor_profile` 任务，确保报告信息卡片有明确的公开资料证据支撑。收集分支携带一条干净的种子查询，同时将竞品、维度、来源提示、成功标准及任务上下文保存为结构化字段。
...
**report_writer**（ReportWriterAgent）将 Rivalens 论断、`CompetitorKnowledge` 和已接受的 `EvidenceItem` 记录适配到共享的 `ReportGenerator` 写作路径。使用固定的报告契约，包含 SWOT/TOWS 矩阵骨架，分析章节维度动态派生自有支撑的论断和已接受证据。
...
下游智能体通过 `latest_message_for(...)` 消费发给自己的最新已验证消息，使每条 DAG 边更像类型化的函数调用契约，而非自由文本。
...
CollectionAgent
  ...
  -> SourceMetricsBuilder（已接受证据来源独立性指标）
  -> CoverageReviewer（标准覆盖缺口、LLM 建议的来源缺口）
  -> BranchCoverageStateBuilder（根分支/组覆盖台账）
```

### 收集质量闭环

- **EvidenceQualityReviewer** 在每次标准搜索后立即运行，生成包含接受/拒绝证据 ID、成功标准匹配、发现、评分和所需操作的 `EvidenceReviewResult` 记录。
- **SourceMetricsBuilder** 计算确定性的已接受来源指标：`accepted_evidence_count`、`unique_canonical_url_count`、`unique_domain_count`、`independent_source_count`、`primary_source_count` 及重复来源组。
- **CoverageReviewer** 消费审查结果并控制分支级覆盖：来源类型缺口、已满足/部分满足/缺失的标准、缺失的引导问题，以及缺口驱动的跟进任务规格。LLM 来源缺口顾问（`LLMSourceGapAdvisor`）判断已接受证据的来源组合是否需要针对性跟进。
- **BranchCoverageStateBuilder** 将每个根分支及其跟进子分支聚合为 `branch_coverage_states`，记录缺口代码、已解决/已阻塞的缺口记录、父子改进评估及最终覆盖状态。
...
`ScrapedSourceCache`（SQLite）按规范 URL（去除片段和常见追踪参数）缓存页面。缓存命中返回与实时爬取相同的原始页面结构，但缓存页面仍须正常通过 `EvidenceQualityReviewer` 和 `CoverageReviewer`。证据条目携带 `canonical_url`、`source_domain`、`scraped_content_sha256` 和 `source_cache.status` 元数据。
...
`CollectionAgent` 在查询到达检索器之前对其进行了本地化处理。如果原始任务使用中文竞品名称（如飞书、钉钉），规划的子查询会使用中文来源术语（官网、定价、文档、评价、新闻），使发现偏向中文来源。

## PostgreSQL 数据

PostgreSQL 存储用户认证和持久化业务溯源。LangSmith 负责详细的执行可观测性（模型/工具跨度、提示、输出、延迟、Token 用量、成本）。PostgreSQL 存储回放和可视化"报告为何包含某条论断"所需的领域关系。

### 用户表

`users` 表在启动时由 `backend/server/user_store.py` 创建（SQL 见 `backend/server/sql_table_create/001_users.sql`）：

- `id` — 稳定 UUID 标识
- `email` — 归一化小写登录标识（唯一索引）
- `display_name` — 用户显示名称
- `password_hash` — 加盐 scrypt 摘要（绝不存储明文）
- `role` 和 `status` — 授权角色与账户可用性
- `email_verified_at`、`last_login_at` — 验证与登录审计
- `created_at`、`updated_at` — 账户审计时间戳

### 溯源链

`backend/server/trace_store.py` 创建溯源表（SQL 见 `backend/server/sql_table_create/002_traceability.sql`）。每次 Rivalens 运行在执行前获得 `running` 记录；完成的运行在工作流边界事务性存储；失败的运行在数据库可用时保留可审计的 `analysis_runs` 状态。
...
数据库必须可用 pgvector 扩展；Alembic 在启动迁移期间执行 `CREATE EXTENSION IF NOT EXISTS vector`。
...
顶层 LangGraph 运行带有 `rivalens`、`competitive-analysis` 标签及检索器、分支预算、竞品数量和收集归属等元数据。

关键追踪节点：`rivalens_scope_planner`（竞品范围、行业、方向）、`rivalens_collect_evidence`、`rivalens_process_subquery`、`rivalens_retriever_search`、`rivalens_scrape_url`、`rivalens_evidence_quality_review`、`rivalens_coverage_review`。
...
`KnowledgeStructuringAgent` 使用确定性规则从已接受证据中提取结构化 `KnowledgeFact` 原子事实：

- 跳过语义噪声，裁剪上下文窗口，在限定词中保留来源标题和 URL
- 引用已接受输入 `EvidenceItem.id`
- 将定价证据原子化为免费版、套餐价、询价制、按量计费和年度折扣事实
- 智能体事件记录规则输入数、跳过数、语义噪声数、上下文裁剪数、事实数及原子化计数

在构建 `EvidenceItem` 记录之前，`ResearchEngineEvidenceCollector` 清洗爬取的源内容：移除 HTML 导航框架、JavaScript 回退文本、无效日期噪声（`NaN-NaN-NaN`）和 AI 关键词匹配提示。原始爬取内容哈希保留在 `scraped_content_sha256` 中；仅对 EvidenceItem 摘录进行清洗以供下游审查。

`KnowledgeStructuringAgent` 还在事实提取之前用 top-k `evidence_snippets`（针对匹配成功标准的句子级支撑）丰富已接受证据。
...
LLM 对每个"竞品 × 维度"的 `KnowledgeFact` 包发起一次请求。它可以选择哪些 `knowledge_fact_ids` 支撑每条论断，但 `evidence_ids`、风险级别、归一化键和报告路由仍在本地绑定。失败的包回退至规则生成的论断。
...
报告类型（通过 API 请求的 `report_type` 参数配置）：`research_report`（摘要，约 2 分钟）、`detailed_report`（详细，约 5 分钟）、`deep_research`、`rivalens`（完整多智能体工作流）等。
