...
当报告来源选择“我的文档”或“混合”时，偏好设置弹窗支持上传文件辅助分析。服务端会校验上传路径必须位于 `DOC_PATH` 内，将文件路径传入 Rivalens 工作流，把相关文件片段转换为本地 `EvidenceItem`，并沿用与网页来源相同的证据到结论追踪链。当前轻量文件上下文支持 CSV、Excel、JSON 以及常见截图/图片格式。
...
| GET | `/files/` | 列出已上传文件 |
| GET | `/files/{filename}` | 下载本地证据引用的上传文件 |
