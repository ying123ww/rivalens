...
export const getHost = ({ purpose }: GetHostParams = {}): string => {
  if (typeof window !== 'undefined') {
    ...
    const apiUrlInLocalStorage = localStorage.getItem("RIVALENS_API_URL");
    
    const urlParams = new URLSearchParams(window.location.search);
    const apiUrlInUrlParams = urlParams.get("RIVALENS_API_URL");
    
    if (apiUrlInLocalStorage) {
      return apiUrlInLocalStorage;
    } else if (apiUrlInUrlParams) {
      return apiUrlInUrlParams;
    } else if (process.env.NEXT_PUBLIC_RIVALENS_API_URL) {
      return process.env.NEXT_PUBLIC_RIVALENS_API_URL;
    } else if (process.env.REACT_APP_RIVALENS_API_URL) {
      return process.env.REACT_APP_RIVALENS_API_URL;
