...
class DetailedReport:
    def __init__(
        ...
        if mcp_configs is not None:
            research_params["mcp_configs"] = mcp_configs
        if mcp_strategy is not None:
            research_params["mcp_strategy"] = mcp_strategy

        self.research_engine = ResearchEngine(**research_params)

        # Override max_search_results_per_query if provided by user
        if max_search_results is not None:
            self.research_engine.cfg.max_search_results_per_query = int(max_search_results)
    ...
    async def run(self) -> str:
        ...
        report_introduction = await self.research_engine.write_introduction()
        _, report_body = await self._generate_subtopic_reports(subtopics)
        self.research_engine.visited_urls.update(self.global_urls)
        report = await self._construct_detailed_report(report_introduction, report_body)
        return report

    async def _initial_research(self) -> None:
        await self.research_engine.conduct_research()
        self.global_context = self.research_engine.context
        self.global_urls = self.research_engine.visited_urls

    async def _get_all_subtopics(self) -> List[Dict]:
        subtopics_data = await self.research_engine.get_subtopics()
    ...
    async def _get_subtopic_report(self, subtopic: Dict) -> Dict[str, str]:
        ...
        subtopic_assistant = ResearchEngine(
            ...
            agent=self.research_engine.agent,
            role=self.research_engine.role,
            tone=self.tone,
            complement_source_urls=self.complement_source_urls,
            source_urls=self.source_urls,
            # Propagate MCP configuration so follow-up researchers can use MCP
            mcp_configs=self.research_engine.mcp_configs,
            mcp_strategy=self.research_engine.mcp_strategy
        )
        ...
        parse_draft_section_titles = self.research_engine.extract_headers(draft_section_titles)
        ...
        self.global_written_sections.extend(self.research_engine.extract_sections(subtopic_report))
        self.global_context = list(set(self._hashable_context(subtopic_assistant.context)))
        self.global_urls.update(subtopic_assistant.visited_urls)

        self.existing_headers.append({
            "subtopic task": current_subtopic_task,
            "headers": self.research_engine.extract_headers(subtopic_report),
        })
    ...
    async def _construct_detailed_report(self, introduction: str, report_body: str) -> str:
        toc = self.research_engine.table_of_contents(report_body)
        conclusion = await self.research_engine.write_report_conclusion(report_body)
        conclusion_with_references = self.research_engine.add_references(
            conclusion, self.research_engine.visited_urls)
